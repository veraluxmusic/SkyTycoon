...
{
    internal static class GradientTable2D
    {
        public static void Build(NativeArray<float2> destination)
        {
            if (destination.Length != Constants.GradientTableSize2D)
                throw new ArgumentException($"Gradient table must contain exactly {Constants.GradientTableSize2D} entries.", nameof(destination));
